...
class Admin::EventRegistrationsController < AdminController
  ...
  def index
    ...
    # end
    # binding.pry
    @registrations = @registrations.where( :id => params[:registration_id].split(",") ) if params[:registration_id].present?
    @registrations = @registrations.where( "created_at >= ?", Date.parse(params[:start_on]).beginning_of_day ) if params[:start_on].present?
    @registrations = @registrations.where( "created_at <= ?", Date.parse(params[:end_on]).end_of_day ) if params[:end_on].present?
